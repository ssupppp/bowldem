        ...
        {!archiveMode && (
          isAuthenticated ? (
            <div className="auth-signed-in-badge">
              <span>✓</span>
              <span>Saved to your leaderboard profile</span>
            </div>
          ) : (
            <div className="auth-prompt-section" style={{ marginTop: '0.5rem' }}>
              <p className="auth-prompt-text">Save this win to your leaderboard profile</p>
              <button className="auth-prompt-btn" onClick={() => { setShowSuccessModal(false); setShowAuthModal(true); }}>
    ...
    return (
      <div className={`result-modal result-modal-failure ${answerRevealed ? 'result-modal-reveal' : 'result-modal-compact'}`}>
        ...
        {answerRevealed ? (
          ...
          <>
            {/* Auth prompt — drives sign-ups on loss */}
            {!isAuthenticated ? (
              <div className="auth-prompt-section" style={{ marginTop: '0.5rem' }}>
                <p className="auth-prompt-text">Sign in to get the answer in your email</p>
                <button className="auth-prompt-btn" onClick={() => { setShowGameOverModal(false); setShowAuthModal(true); }}>
                  <svg width="16" height="16" viewBox="0 0 24 24">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                ...
                <p className="auth-footer-note" style={{ marginTop: '8px' }}>One tap. No passwords.</p>
              </div>
            ) : (
              <div className="auth-signed-in-badge">
                <span>✓</span>
                <span>Answer coming to your email</span>
              </div>
            )}
          </>
        )}
